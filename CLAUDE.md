...
- **ВОЛНА G «РЕЗЕРВ МОЗГА НА ПОДПИСКЕ» (2026-08-31, по просьбе владельца «апи кончилось»):** основной
  ...
  **fable**), `JARVIS_SUBSCRIPTION_EFFORT` (деф **max**).
  **МОДЕЛЬ И ЭФФОРТ РЕЗЕРВА (решение владельца 2026-08-31):** сильная модель + максимум — резерв
  включается, когда API уже недоступен, то есть это ПОСЛЕДНИЙ шанс сделать ход правильно, а лимиты
  подписки уже оплачены. Живым зондом подтверждено, что на подписке доступны `fable`→claude-fable-5
