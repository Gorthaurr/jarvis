...
- **ВОЛНА G «РЕЗЕРВ МОЗГА НА ПОДПИСКЕ» (2026-08-31, по просьбе владельца «апи кончилось»):** основной
  ...
  **МОДЕЛЬ И ЭФФОРТ РЕЗЕРВА (решение владельца 2026-08-31):** модель — **Opus 5** (`opus`), эффорт —
  ПО ТИРУ (обычный ход `low`, тир fable `max`). Почему Opus 5, а не Fable 5: замер показал, что
  латентность от модели НЕ зависит (её держит оверхед SDK ~5с), а лимит подписки ОБЩИЙ с Claude Code
  владельца — значит платить сильнейшей моделью за каждую реплику незачем. Fable 5 доступен флагом. Живым зондом подтверждено, что на подписке доступны `fable`→claude-fable-5
  и `opus`→claude-opus-5 (полные id тоже принимаются); тир хода на выбор модели в резерве НЕ влияет.
