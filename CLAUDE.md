...
- **ВОЛНА F «адаптация OpenClaw 2.0» (2026-08-31; вердикты разбора — `docs/OPENCLAW_ADOPTION_2026-08-31.md`;
  ...
  sha256(what|condition|action) РОВНО того, что владелец одобрил на confirm при постановке; сверяется
  ПЕРЕД КАЖДЫМ исполнением действия (dispatchAction И flushPendingActions — между парковкой и коннектом
  запись тоже могла измениться): разошёлся → действие НЕ исполняется, ОДНО честное уведомление
  «поручение изменилось после одобрения», watch → suspended. predicate ВНЕ отпечатка СОЗНАТЕЛЬНО
  (его tabId/url легитимно патчится self-heal'ом вкладки 2026-07-25); легаси-записи без отпечатка —
  grandfather (ретро-блок сломал бы обещанные поручения). (б) **согласия §14 перестали быть невидимыми**:
  ...
  `owner` был мёртв (объявлен, недостижим) → убран до появления UI-ввода факта. Тесты: сервер 1969.
