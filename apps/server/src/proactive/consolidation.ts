...
export function looksLikeDirective(text: string): boolean {
  ...
  // Императивы пересылки/автодействий/раскрытия секретов (подстрока — \b на кириллице ненадёжен).
  if (/пересыла|перенаправля|forward|отправляй\s+(все|всё|каждо)|распорядил|раскро|отправь\s+ключ|пароль|token|api[_-]?key/i.test(t)) return true;
  return false;
}
