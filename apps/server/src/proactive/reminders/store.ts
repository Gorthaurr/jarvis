...
export class ReminderStore {
  ...
  markFiredUndelivered(id: string, firedAt: number): void {
    ...
    if (!r) return;
    r.firedAt = firedAt;
    // ВОЗВРАТ ИЗ «done» (контроль-10): при откате доставки (реплику приняли в очередь, но она не
    // прозвучала — «стоп»/смерть сессии) статус уже стоял `done`, а `awaitingDelivery` смотрит ТОЛЬКО
    // на `scheduled` → откат был НО-ОПОМ, напоминание терялось навсегда, а лог рапортовал «ждёт
    // доставки». Возвращаем ровно из `done`: `cancelled` воскрешать нельзя — владелец её снял сам
    // (а «стоп» дёргает как раз сброс очереди, и отмена не должна ожить этим путём).
    if (r.status === "done") r.status = "scheduled";
    this.persist();
  }
