...
export class WatchService {
  ...
  /**
   * ОДНО честное «не могу наблюдать X» (и до восстановления — молчим).
   *
   * Идёт ТЕМ ЖЕ notify(), что и срабатывания: там уже живёт дисциплина доставки волны D
   * (retriable + onOutcome, «принято ≠ прозвучало», парковка в pendingNotify при отказе очереди).
   * Своим каналом это уведомление молча терялось бы ровно как те, ради которых дисциплину и вводили.
   */
  private reportBlindness(w: Watch): void {
    if (w.blindNotifiedAt !== undefined) return; // уже сказали — не спамим каждый тик
    const now = this.now();
    if (!isBlind(w, now)) return;
    const hours = Math.max(1, Math.round((now - lastOkSince(w)) / 3600_000));
    w.blindNotifiedAt = now;
    log.warn("наблюдение ОСЛЕПЛО: нет удачных проверок дольше порога", { id: w.id, hours, since: lastOkSince(w) });
    this.notify(
      w,
      w.lastOkAt === undefined
        ? // 🔴 Говорим ровно то, что ЗНАЕМ (адверс-ревью): «удачных проверок не записано» ≠ «ни одна
          // проверка не прошла». Поле lastOkAt появилось позже самих наблюдений, поэтому у записи из
          // прошлых волн его нет ДАЖЕ ЕСЛИ она исправно проверялась (в watches.json остались lastCheckAt
          // и lastValue). Прежняя формулировка утверждала выдуманный факт о собственной истории —
          // владелец слышал, что за заказом не следили НИ РАЗУ, и снимал живое наблюдение.
          `Не могу наблюдать «${w.what}»: источник недоступен, а удачных проверок у меня не записано (наблюдение поставлено ${hours} ч назад). Наблюдение не снимаю, но пока я слеп, сэр.`
        : `Не могу наблюдать «${w.what}»: последняя удачная проверка была ${hours} ч назад, с тех пор источник недоступен. Наблюдение не снимаю, но пока я слеп, сэр.`,
    );
  }
  ...
  private async runCheck(w: Watch): Promise<void> {
    ...
    if (res.error) {
      ...
      if (res.transient) {
        ...
        // 🔴 …но МОЛЧАТЬ о слепоте нельзя (живой сценарий: Chrome закрыт на ночь → 12 часов «слежу»
        // без единой удачной проверки, а владелец утром уверен, что за заказом следят). Транзиент
        // по-прежнему не копится к suspend — наблюдение живо и ждёт возврата источника, но факт
        // «я не вижу цель с такого-то времени» обязан быть сказан ОДИН раз.
        this.reportBlindness(w);
        this.store.update(w);
        return;
      }
    ...
    // Источник ответил — значит наблюдение ЗРЯЧЕЕ. Флаг доклада снимаем: восстановились, и следующая
    // слепота заслуживает нового честного слова (иначе одно ночное «не вижу» глушило бы все будущие).
    w.lastOkAt = this.now();
    w.blindNotifiedAt = undefined;
    if (res.met) {
