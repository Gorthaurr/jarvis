...
describe("db/credentials — per-user шифр-ключи (PGlite)", () => {
  ...
  // 🔴 Аудит тестовой базы 2026-09-01: имя этого теста обещало гард, а тело утверждало обратное —
  // ключ самобутстрапится, поэтому до ветки отказа тем путём не дойти. Имя приведено в соответствие
  // с тем, что тест реально проверяет; сам гард «нет ключа — не сохраняем» покрыт в credentials-nokey.test.ts.
  it("мастер-ключ самобутстрапится из keyfile: без env-переменной сохранение всё равно шифрованное", async () => {
    delete process.env.CREDENTIALS_MASTER_KEY;
    const dir = `${process.env.TEMP || "/tmp"}/jarvis-nokey-${process.pid}-${Date.now()}`;
    // Ключ есть всегда (самобутстрап из keyfile), поэтому здесь проверяется именно это: даже без
    // env-переменной шифрование доступно и сохранение идёт.
