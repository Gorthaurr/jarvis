...
export interface LlmRequest {
  ...
  cachePrefix?: boolean;
  /**
   * W2 (2026-09-09): ключ сессии провайдера — id задачи. Провайдер, умеющий держать сессию (подписка
   * через Agent SDK), продолжает ОДИН диалог с моделью между раундами петли, вместо того чтобы на каждом
   * раунде поднимать новый процесс и пересылать историю. Петля ОБЯЗАНА позвать `release(key)` в finally.
...
export interface LlmResponse {
   ...
   * канала (у владельца `claude-opus-4-8`), которую никто не вызывал. До 2026-09-02 метрики и логи
   * писали именно `req.model`, и на прямой вопрос «там точно Opus 5?» лог отвечал неправдой.
   * Не задано — ход выполнен запрошенной моделью.
   */
  modelUsed?: string;
  /**
...
export interface ILlmProvider {
  ...
  /**
   * Состояние каналов мозга — реализует фолбэк-цепочка (FallbackLlmProvider), у одиночных
   * провайдеров отсутствует. Нужно паспорту возможностей: модель должна ЗНАТЬ, что работает по
   * резерву и почему основной канал выключен, а не обещать владельцу скорость, которой сейчас нет.
   */
...
/**
 * Снимок каналов мозга (волна G + разбор 2026-09-02). `primary`:
 *  • "ok" — основной канал (API по ключу) в работе;
 *  • "cooldown" — транзиентный сбой, короткая пауза, скоро пробуем снова;
 *  • "off" — канал ВЫКЛЮЧЕН: нет ключа, выключен настройкой, или отказ, который повтором не лечится
...
export interface LlmChannelStatus {
  ...
  kind?: string;
  /** Причина человеческим языком — идёт в паспорт возможностей и во фразу владельцу. */
  human?: string;
  /** Жив ли резервный канал на подписке (есть ли чем работать, пока основной выключен). */
