...
export interface AgentMetricEvent {
  ...
  ok: boolean;
  /**
   * ПОЧЕМУ провал (2026-08-31). Разбор боевой телеметрии показал: треть провалов — ходы, которые
   * вообще НЕ ДОШЛИ до модели (кончился ключ, протухла подписка): 0 раундов, 0 выходных токенов,
   * полсекунды. Считать их «Джарвис не справился» — та же подмена, что «не смог проверить» = «не
   * получилось»: самодиагностика начинает винить себя за отсутствие связи.
   *  • `llm_unavailable` — канал модели не ответил (стаб);
