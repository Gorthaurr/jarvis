...
describe("выученные навыки-процедуры (§8 HERMES)", () => {
  ...
  const LEARNED: RecalledSkill[] = [
    { id: "tg", ownerId: "u-1", name: "Отчёт в Telegram", when: "прислать отчёт в телеграм", procedure: "...", version: 1 },
    { id: "excel", ownerId: "u-1", name: "Свести таблицу", when: "посчитать сумму в экселе", procedure: "...", version: 1 },
  ];
  ...
  it("matchLearnedSkill: чужие слова с коротким общим префиксом НЕ матчатся (стемминг длинозависимый)", () => {
    const collide: RecalledSkill[] = [
      { id: "stol", ownerId: "u-1", name: "Столица", when: "столовые приборы", procedure: "...", version: 1 },
      { id: "post", ownerId: "u-1", name: "Почта", when: "почтовая марка", procedure: "...", version: 1 },
    ];
  ...
  it("matchLearnedSkill: порог 0.34 — 2 попадания из 6 целей (0.333) ниже порога", () => {
    const six: RecalledSkill[] = [
      { id: "six", ownerId: "u-1", name: "Открыть Notion базу заметок проекта плана", when: "", procedure: "...", version: 1 },
    ];
  ...
  it("matchLearnedSkill: тай-брейк по id детерминирован при равном счёте", () => {
    const tie: RecalledSkill[] = [
      { id: "bbb", ownerId: "u-1", name: "Отчёт телеграм", when: "", procedure: "...", version: 1 },
      { id: "aaa", ownerId: "u-1", name: "Отчёт телеграм", when: "", procedure: "...", version: 1 },
    ];
  ...
  it("matchLearnedSkill P2.3: навык с failCount ≥ порога (3) НЕ подсовывается (учится на ошибках)", () => {
    const flaky: RecalledSkill[] = [
      { id: "tg", ownerId: "u-1", name: "Отчёт в Telegram", when: "прислать отчёт в телеграм", procedure: "...", version: 1, failCount: 3 },
    ];
...
describe("КОРОТКИХ ИМЁН ГРАНИЦА (адверсариал-проверка)", () => {
  it("matchLearnedSkill: 3-символьное имя 'Лев' / 'лев' ОТСЕКАЕТСЯ жадной фильтрацией", () => {
    const skills: RecalledSkill[] = [
      { id: "msg_lev", ownerId: "u-1", name: "Написать Льву", when: "когда нужно написать личное сообщение", procedure: "...", version: 1 },
    ];
  ...
  it("matchLearnedSkill: 3-символьное имя 'Ада' ОТСЕКАЕТСЯ совсем", () => {
    const skills: RecalledSkill[] = [
      { id: "msg_ada", ownerId: "u-1", name: "Написать Аде", when: "когда нужно написать", procedure: "...", version: 1 },
    ];
  ...
  it("Граница 4 символа: 'льву' и 'лев' - это разные слова для стемминга (prefix-матч = min 5 символов)", () => {
    ...
    const skills: RecalledSkill[] = [
      { id: "msg_lev", ownerId: "u-1", name: "Написать Льву", when: "", procedure: "...", version: 1 },
    ];
...
describe("recallSemantic (§8 семантический recall навыка)", () => {
  ...
  const SKILLS: RecalledSkill[] = [
    { id: "search-dota", ownerId: "u-1", name: "Запустить поиск в Доте", when: "начать матч в доте", procedure: "...", version: 1 },
    { id: "send-tg", ownerId: "u-1", name: "Отправить в телеграм", when: "переслать сообщение", procedure: "...", version: 1 },
  ];
  ...
  it("M8 (security): кэш векторов триггеров скоупится по владельцу — одинаковый id разных юзеров НЕ делит вектор", async () => {
    // Два разных навыка (разные юзеры) с ОДИНАКОВЫМ id (slug совпал, напр. общий "learned__open-notion") и
    // одинаковой version — если кэш ключуется голым id, второй recall получит вектор ПЕРВОГО юзера (утечка
    // через listSkillsMerged). Триггер-тексты сделаны разными, чтобы протухший кэш дал ложный высокий косинус.
    let calls = 0;
    const emb = new FakeEmb({
      "Навык А. открыть личный проект А": [1, 0, 0],
      "Навык Б. открыть личный проект Б": [0, 1, 0],
      "открой мой личный проект а": [0.99, 0.05, 0],
      "открой мой личный проект б": [0.05, 0.99, 0],
    });
    const embedCounting: IEmbeddingProvider = {
      dim: 3,
      live: true,
      async embed(text, kind) {
        calls += 1;
        return emb.embed(text, kind);
      },
    };
    const same_id = "learned__shared-slug";
    const skillA: RecalledSkill = { id: same_id, ownerId: "user-a", name: "Навык А", when: "открыть личный проект А", procedure: "...", version: 1 };
    const skillB: RecalledSkill = { id: same_id, ownerId: "user-b", name: "Навык Б", when: "открыть личный проект Б", procedure: "...", version: 1 };

    // Recall юзера A прогревает кэш триггер-вектора под "user-a:learned__shared-slug".
    const rA = await recallSemantic(embedCounting, "открой мой личный проект а", [skillA]);
    expect(rA?.ownerId).toBe("user-a");
    const callsAfterA = calls;

    // Recall юзера B с ТЕМ ЖЕ id, но своим триггером — если бы ключ был голым id, тут отдался бы кэш A
    // (вектор [1,0,0]) и совпадение "проект б" с триггером A было бы ниже порога → ложный null/чужой навык.
    const rB = await recallSemantic(embedCounting, "открой мой личный проект б", [skillB]);
    expect(rB?.ownerId).toBe("user-b");
    expect(rB?.id).toBe(same_id);
    // Кэш-промах для B доказывает отдельный слот (composite-ключ), не переиспользование вектора A.
    expect(calls).toBeGreaterThan(callsAfterA);
  });
});
