...
export interface RecalledSkill {
  ...
  needsReview?: boolean;
  /** §P0 (гейт авто-реплея): семантическая уверенность recall (гибридный ранг e5+бонусы), проставляется
   *  recallSemantic на попадании. ОТСУТСТВУЕТ у лексического матча — слепой авто-реплей без
   *  семантической уверенности запрещён (см. brain/agent/replay-gate.ts). */
  recallSim?: number;
