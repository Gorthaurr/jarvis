...
describe("SpendGuard — санитизация предохранителей (§14)", () => {
  ...
  // Монотонность hydrate (фикс M3) проверяется в hydrate-monotonic.test.ts — там БД подменена и
  // ветка реально исполняется. Прежний тест здесь работал вхолостую (без БД query() — no-op) и
  // снятие фикса не ловил; аудит тестовой базы 2026-09-01.
