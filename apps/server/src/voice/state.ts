  ...
  switch (ctx.state) {
    ...
    case "thinking": {
      switch (ev.type) {
        ...
        case "barge_in":
          // УСТОЙЧИВОЕ перебивание (клиент подтвердил речь ≥BARGE_SUSTAIN_MS) — отменяем запущенный ход
          // агента (cancel_tts инкрементит gen) и слушаем заново (§10).
          return go("listening", false, [{ type: "cancel_tts" }, { type: "open_stt" }]);
        case "speech_start":
          // 🔴 СЫРОЙ VAD В РАЗДУМЬЕ НЕ ОТМЕНЯЕТ ХОД (живой корень «не договаривает/молчит», 2026-07-24).
          // Раньше здесь стоял cancel_tts: ЛЮБОЙ звук в комнате (кашель, ТВ, чужая реплика, эхо) во время
          // thinking бампал gen → уже сгенерированный ответ выбрасывался ЦЕЛИКОМ и молча — ни голоса, ни
          // текста в чат. Эмпирика: 7 из 23 реплик за сессию не зазвучали, и ВСЕ потерянные имели
          // переоткрытие STT (маркер cancel_tts) внутри окна раздумья, а все озвученные — ни одного.
          // Хуже: владелец повторял команду → дубль отправки в Telegram.
          // Теперь микрофон открываем (слушаем — вдруг это правда команда), но ХОД НЕ УБИВАЕМ: реальное
          // намерение перебить придёт как barge_in (устойчивая речь) либо как transcript_final с
          // обращением — тогда отмена отработает штатно.
