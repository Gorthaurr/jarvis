...
describe("isOfferDeclined — только ГОЛЫЙ отказ от предложения", () => {
  ...
  // 🔴 Контроль-6: словарь отказа был ВТОРОЙ КОПИЕЙ классификатора и разошёлся — канонические
  // cancel-фразы проекта он не узнавал, и отклонённая работа воскресала по сказанному ПЛЕЕРУ «продолжи».
  it("канонические cancel-формы проекта тоже считаются отказом", () => {
    for (const t of ["забудь про это", "забудь об этом", "не надо больше", "больше не надо", "отмени задачу", "прекрати задачу", "прерви", "прекратить"])
      expect(isOfferDeclined(t), t).toBe(true);
  });
