...
export interface TurnRunners {
  runTier0: (session: Session, local: LocalIntent, deps: AgentDeps, sink?: ReplySink, followup?: { goal: string; freshContext?: boolean; viaWake?: boolean; machine?: boolean }) => Promise<AgentReply>;
...
export async function interceptClarify(t: TurnCtx): Promise<AgentReply | null> {
  ...
  if (deps.pendingClarify && !machineTurn) {
    ...
    if (resolved) {
      ...
      const t0c = await runTier0(session, resolved, deps, sink, { goal: clean, freshContext: t.freshContext, viaWake: t.meta?.viaWake, machine: machineTurn }); // sink → консьерж-открытие тоже sync-first
