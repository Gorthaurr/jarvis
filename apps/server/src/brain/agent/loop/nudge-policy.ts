...
export function goalCheck(ctx: LoopCtx, resp: LlmResponse, snap: RoundSnapshot): boolean {
  ...
  // Ревью 2026-09-24 (живой прогон): goal-check ловит деградацию ДЕЙСТВИЯ до подцели («запустил Доту» вместо
  // «запусти поиск»). На ВОПРОСЕ и на ходе из одних чтений (поиск/чтение, ни одной мутации) сверять нечего — а
  // нудж выбрасывал готовый ответ: «какая погода завтра?» → прогноз → «цель достигнута?» → новый раунд подписки
  // с нуля (16,8 с) → «Задача выполнена, сэр — прогноз получен и озвучен» ВМЕСТО прогноза.
  ...
  // W1 (L-3): но «заявка только о запуске» — лишь пока в задаче НЕТ сверенного не-запускного дела. Регэксп ищет
  // «открыл/запустил» в ЛЮБОМ месте текста, и честное «Открыл блокнот и напечатал X» после act verified:"met"
  // шло лишним раундом (на подписке — новая сессия) при уже подтверждённом деле.
  const launchOnlyClaim = !st.honesty.verifiedRealAction && /(?<![\p{L}])(запущен|запустил|поднялс|стартовал|открыл)\p{L}*/iu.test(resp.text || "");
  ...
  if (resp.stopReason === "end_turn" && !st.honesty.goalCheckDone && st.progress.round >= 2 && (!st.honesty.lastRoundHadVerify || launchOnlyClaim) && !goalCheckRedundant) {
    ...
    pushSystemNote(
      `Стоп — сверься с ИСХОДНОЙ задачей: «${text}». Выполнена ли она ЦЕЛИКОМ, или сделана только ` +
        ...
        `скажи владельцу итог одной-двумя фразами (ЧТО именно сделано и видно), без канцелярита «задача выполнена».`,
    );
