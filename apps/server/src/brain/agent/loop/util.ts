...
/**
 * Ревью 2026-09-24 (T-F6/B-F1): sink, которому важно «модель пошла в инструменты» — sync-first промотирует ход
 * в фон по ФАКТУ первого tool_use, а не по таймеру (раунд подписки ≥2,9 с: таймер 1,5 с давал «Берусь» на
 * КАЖДОЕ действие, даже на текстовый ответ). Поле необязательное: остальные sink'и его не знают.
 */
export interface ToolRoundAwareSink extends ReplySink {
  onToolRound?(): void;
}
export function notifyToolRound(sink: ReplySink | undefined): void {
  (sink as ToolRoundAwareSink | undefined)?.onToolRound?.();
}
...
/**
 * Ревью 2026-09-24 (T-F2): навыки, чья подсказка в промпт НЕ положена (шумный recall / реплика не команда /
 * разговорный ход). Сам recall при этом живёт дальше — реплей-гейт и учёт исхода видят его как прежде.
 * Метка по ИДЕНТИЧНОСТИ объекта хода (retrieval кладёт сюда свою копию) — поэтому WeakSet: пересобирать
 * контракт prompt/context ради одного флага не пришлось (их правит интегратор, см. retrieval.ts).
 */
const SKILL_HINT_SUPPRESSED = new WeakSet<RecalledSkill>();
export function suppressSkillHint(s: RecalledSkill): void {
  SKILL_HINT_SUPPRESSED.add(s);
}
 ...
 * Пустая строка = блока нет (persona не рисует пустой learnedSkill).
 */
export function formatRecalledSkill(s: RecalledSkill): string {
  if (SKILL_HINT_SUPPRESSED.has(s)) return "";
  return [
    ...
    s.procedure,
    "",
    // §8: если просьба — ИМЕННО про это действие, навык исполняется инструментами, а не пересказывается
    // (наблюдённый баг с Дотой). Ревью 2026-09-24 (T-F2): прежнее «всё равно ДЕЙСТВУЙ, не отвечай болтовнёй»
    // превращало ЧУЖОЙ recall в приказ — на «нет, не надо» Джарвис лез в браузер. Неподходящий навык теперь
    // игнорируется ПОЛНОСТЬЮ: recall на шумном e5 не идеален, и решает модель, а не совпадение косинуса.
    "Если просьба владельца — ИМЕННО про это действие: исполни навык инструментами в ЭТОМ ходе " +
      "(не пересказывай словами), следуй шагам гибко и проверь исход глазами (screen_capture/browser_read). " +
      "Если навык не про эту просьбу — игнорируй его ПОЛНОСТЬЮ: ничего по нему не открывай и не запускай, " +
      "просто ответь на то, что сказано.",
  ].join("\n");
}
