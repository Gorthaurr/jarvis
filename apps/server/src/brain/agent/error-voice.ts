...
// ⚠️ ЗДЕСЬ БЫЛ regex-детектор «заявил о проверке, не вызвав инструментов» (эпизод 2026-07-25: «Проверил
// историю — не отправлял», rounds:0/toolCalls:0). УБРАН по итогам адверс-ревью — эвристика оказалась и
// вредной, и бесполезной: (1) ловила ОТРИЦАНИЕ и чужие проверки («я НЕ проверял историю», «вы сказали,
// что проверили») — то есть карала ровно те честные формулировки, которым учит persona v79; (2) ключи
// шли подстроками («лог» ⊂ логика/каталог/диалог, «данн» ⊂ «в данном случае», «чат» ⊂ начать/печать) —
// 10 из 10 нейтральных реплик давали ложный матч и лишний LLM-раунд; (3) на ГОЛОСОВОМ разговорном пути
// step-0-текст стримится в синтез ДО терминала, поэтому нудж срабатывал уже ПОСЛЕ произнесения лжи —
// исправление уходило только в чат, создавая расхождение «сказал одно, в истории другое».
// Проблема лечится не текстовым детектором, а ПОЧВОЙ: (а) реестр задач помнит 7 суток (было 6 ч —
