 ...
 */
/** Ядро отказа — СЛОВАРЬ КЛАССИФИКАТОРА (единый источник) + «нет/не/неважно». */
const DECLINE_CORE = new Set<string>([...CANCEL_WORDS, "не", "нет", "неважно"]);
/**
 * Допустимая обвязка = токены КАНОНИЧЕСКИХ cancel-фраз проекта («забудь про это», «не надо больше»,
 * «отмени задачу») + вежливость + слова про НАШУ задачу. ⚠️ Строится из `CANCEL_PHRASES`, а не руками:
 * контроль-6 показал, что ручная копия разошлась — классификатор считал «забудь про это» отменой, а
 * гард отказа не считал, и отклонённая работа воскресала по сказанному ПЛЕЕРУ «продолжи».
 */
const DECLINE_FORMS = new Set<string>([
  ...DECLINE_CORE,
  ...CANCEL_PHRASES.flatMap((p) => p.split(" ")),
  "надо", "нужно", "ладно", "хорошо", "ок", "окей", "спасибо", "сэр", "уже", "всё", "все",
  "буду", "хочу", "стоит", "задачу", "задачей", "работу", "дело",
]);
