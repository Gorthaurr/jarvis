...
describe("act → gui.act → ToolResult", () => {
  it("команда уходит клиенту как gui.act с origin user и всеми полями", async () => {
    ...
    await dispatchTool("act", { target: "Отправить", app: "Telegram", verify: { text: "Отправлено" } }, ctx);
    expect(sendAction.mock.calls[0]?.[0]).toMatchObject({ kind: "gui.act", target: "Отправить", app: "Telegram", verify: { text: "Отправлено" }, origin: "user" });
  });
