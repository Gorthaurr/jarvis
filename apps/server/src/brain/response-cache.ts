...
// «напом» вместо «напомн»: стем «напомн» НЕ ловил существительное «напомин-а-ние/напоминания» —
// и «отмени напоминание про таблетки» проходило как «фактический вопрос» (живой прогон волны D:
// команда не исполнилась, владельцу зачитали кэш о состоянии, которого уже нет). Добавлены и другие
// личные изменяемые сущности: список дел/встреч/писем протухает так же.
const STOP_PREFIX = [
  "погод", "потрат", "врем", "баланс", "статус", "открыт", "запущен", "включ", "выключ", "таймер",
  "напом", "будильник", "встреч", "письм", "почт", "календар", "задач", "наблюден", "счет", "счёт",
];
...
const COMMAND_PREFIX = [
  ...
  "отмен", "прекрат", "напомина", "следи", "проверь", "запиши", "запомн", "забудь",
];
