...
describe("classifyTier", () => {
  ...
  it("A1 (форензика 2026-07-14): УСЛОВНОЕ ДЕЙСТВИЕ «когда X — императив» = ЗАДАЧА, не conversational-вопрос", () => {
    // Живой эпизод: «когда видео дойдёт до 35 — перемотать до 11» начиналось с «когда» → ложно уходило
    // в conversational (невидимо scope/дубль-гейту → ×3 «Берусь» на пустые задачи).
    const d = classifyTier("когда видео дойдёт до 35 минуты, перемотай до 11 минуты");
    expect(d.conversational).not.toBe(true);
    expect(classifyTier("если цена биткоина упадёт ниже 60000 — уведоми меня").conversational).not.toBe(true);
    expect(classifyTier("как только матч найдётся, скажи мне").conversational).not.toBe(true);
  });

  /**
   * 🔴 Разбор эпизода «Дота» (2026-09-02, HIGH). Живая реплика «Ну, Джарвис, что, войди в пробу
   * героя.»: филлеры «ну»/«джарвис» срезаются, первым словом остаётся «что» — и прямой ПРИКАЗ уходил
   * разговорным тиром. Цена: нет чипа задачи, нет слота параллельности, задача невидима голосовым
   * «стоп/пауза/что делаешь» и чекпойнту (в data/tasks.json она сохранена с conversational:true).
   * Правим ВЫВОД, а не список филлеров: блоклист частиц принципиально неполон (урок lean-smalltalk).
   */
  it("дискурсивная частица «что,» перед императивом = ПРИКАЗ, а не вопрос", () => {
    expect(classifyTier("Ну, Джарвис, что, войди в пробу героя.").conversational).not.toBe(true);
    expect(classifyTier("Так что, запусти сборку.").conversational).not.toBe(true);
    expect(classifyTier("Джарвис, что, открой ютуб").conversational).not.toBe(true);
    // 🔴 Реверт-чувствительный контроль (ревью 2026-09-02): прежний ассерт «войди в пробу героя»
    // проходил и на коде ДО правки (первое слово не вопросительное) — то есть покрытия не давал.
    // Эта фраза падает и при снятии гарда, и при откате списка глаголов движения.
    expect(classifyTier("Что, зайди в игру").conversational).not.toBe(true);
    expect(classifyTier("Ну что, заходи в игру").conversational).not.toBe(true); // несовершенный вид
  });

  it("гард УЗКИЙ: настоящие вопросы вопросами и остаются", () => {
    // За «что» нет паузы-запятой → местоимение в своей роли, это вопрос (даже с императивом в хвосте).
    expect(classifyTier("Что ты умеешь, покажи").conversational).toBe(true);
    expect(classifyTier("что мне посмотреть вечером").conversational).toBe(true);
    expect(classifyTier("как лучше сделать отчёт").conversational).toBe(true);
    expect(classifyTier("Джарвис, что там по погоде?").conversational).toBe(true);
    // Запятая есть, но дальше ИНФИНИТИВ, а не императив — гипотетика остаётся вопросом.
    expect(classifyTier("что, если удалить папку").conversational).toBe(true);
    // Императив ВВОДНЫМ словом (обрамлён второй запятой) приказом не делает — гард узкий.
    expect(classifyTier("Что, покажи, у нас по деньгам").conversational).toBe(true);
  });

  /**
   * Глаголы движения и несовершенный вид отсутствовали в IMPERATIVE_RE вовсе — значит УСЛОВНЫЕ
   * ДЕЙСТВИЯ с ними («когда матч закончится, выйди из игры») уходили в разговор и не становились
   * задачей-наблюдением. Кейсы взяты ПРОВЕРЕННО дискриминирующие (ревью прогнало их против HEAD).
   */
  it("условное действие с глаголами движения/несовершенного вида — ЗАДАЧА", () => {
    expect(classifyTier("когда матч закончится, выйди из игры").conversational).not.toBe(true);
    expect(classifyTier("как только загрузится, войди в аккаунт").conversational).not.toBe(true);
    expect(classifyTier("когда он напишет, перейди в чат").conversational).not.toBe(true);
    expect(classifyTier("когда начнётся матч, врубай трансляцию").conversational).not.toBe(true);
  });

  /**
   * 🔴 Лог 2026-09-02: «Так, так, ну ПОЧЕМУ-ТО Джарвис, мама в 16 … Заходи, трансляцию врубай.» —
   * прямая команда ушла разговорным тиром (самым дорогим — fable), потому что «почему-то» матчилось
   * как вопросительное «почему»: дефис не буква, и правая граница слова его пропускала. Тот же класс:
   * «что-то тормозит, закрой вкладки», «как-то медленно, ускорь». Неопределённая частица -то/-нибудь/
   * -либо — ЧАСТЬ слова, а не граница.
   */
  it("неопределённая частица («почему-то», «что-то») НЕ делает фразу вопросом", () => {
    expect(classifyTier("Так, так, ну почему-то Джарвис, мама в 16 есть на их сайте. Заходи, трансляцию врубай.").conversational).not.toBe(true);
    expect(classifyTier("почему-то не работает, перезапусти сервер").conversational).not.toBe(true);
    expect(classifyTier("что-то тормозит, закрой лишние вкладки").conversational).not.toBe(true);
    expect(classifyTier("как-то медленно, ускорь").conversational).not.toBe(true);
  });

  it("настоящие вопросительные слова затронуты НЕ были", () => {
    expect(classifyTier("почему так медленно").conversational).toBe(true);
    expect(classifyTier("почему ты так медленно работаешь").conversational).toBe(true);
    expect(classifyTier("зачем это нужно").conversational).toBe(true);
    expect(classifyTier("кто-то заходил в чат?").conversational).toBe(true); // «?» решает, а не первое слово
    // Контроль правой границы слова: «ли» внутри «лишние» вопросом не делает (регрессия экранирования).
    expect(classifyTier("закрой лишние вкладки").conversational).not.toBe(true);
    expect(classifyTier("стоит ли покупать").conversational).toBe(true);
  });

  it("A1: настоящий вопрос «когда …?» БЕЗ императива-команды остаётся conversational", () => {
    expect(classifyTier("когда выйдет новый фильм марвел?").conversational).toBe(true);
    expect(classifyTier("когда лучше сажать помидоры").conversational).toBe(true);
  });

  it("A1 (ревью р2 #7): гипотетика/совет с если/когда + глагол — НЕ задача (вопрос с «?» / «что будет» / «лучше»)", () => {
    expect(classifyTier("если я удалю этот файл, что будет?").conversational).toBe(true);
    expect(classifyTier("если удалить это, что случится").conversational).toBe(true);
    expect(classifyTier("когда лучше сделать бэкап").conversational).toBe(true);
    expect(classifyTier("стоит ли отправить это сейчас?").conversational).toBe(true);
  });

  it("A1 (ревью мультимонитор #8): омоним «пошли» (прош. время) в условном ВОПРОСЕ — не задача", () => {
    expect(classifyTier("когда пошли дожди в этом году").conversational).toBe(true);
  });

  it("A1 (ревью р4): частые разговорные императивы в условном действии распознаются (не теряются как болтовня)", () => {
    for (const t of [
      "когда фильм скачается выруби компьютер",
      "когда придёт смс вруби звук",
      "когда батарея сядет предупреди меня",
      "когда матч начнётся переключись на дискорд",
      "когда закончится рендер запиши результат",
      "когда всё загрузится проверь логи",
    ])
      expect(classifyTier(t).conversational, t).not.toBe(true);
  });

  it("A1 (ревью р3 #2/#3): ИНФИНИТИВ в условии-вопросе не путается с императивом-командой", () => {
    // Русский императив/инфинитив делят стем — детект теперь по ИМПЕРАТИВНОЙ форме, не по стему.
    expect(classifyTier("если удалить эту папку что произойдёт").conversational).toBe(true); // инфинитив → вопрос
    expect(classifyTier("если отправлю это сейчас что произойдёт").conversational).toBe(true);
    // а настоящий императив в условном действии — по-прежнему задача
    expect(classifyTier("когда видео дойдёт до 35 минуты, перемотай до 11 минуты").conversational).not.toBe(true);
    expect(classifyTier("как только скачается файл, открой его").conversational).not.toBe(true);
  });
