...
export interface Task {
  ...
  /**
   * Волна H: СОВЕРШЁННЫЕ необратимые действия («сообщение Кате отправлено», «заказ размещён»).
   * Нужны, когда владелец говорит «это была не команда»: отмена останавливает работу, но отправленное
   * не возвращает — и сказать об этом надо прямо, иначе «остановил» читается как «ничего не было».
   */
 ...
 */
export function formatRecentTasks(tasks: readonly Task[], now: number, windowMs?: number): string {
  // ГРАНИЦЫ ПАМЯТИ ЯВНО (живой провал 2026-07-25). На вопрос «ты отправлял ей сообщение?» модель, не
  // найдя записи (вчерашняя отправка была вне окна), ответила категорично: «я не отправлял, это не через
  // меня» — превратив «не вижу» в «этого не было». Список без указания ГРАНИЦ читается как полная
  // история. Поэтому окно называем прямо и запрещаем отрицать факт за его пределами.
  const boundary = windowMs
    ? [
        `⚠️ ВИДНО ТОЛЬКО ЗА ПОСЛЕДНИЕ ${relativeTimeRu(windowMs).replace(/^/, "")} — это НЕ вся твоя история.`,
        "За пределами окна ты своих действий НЕ ВИДИШЬ. Поэтому «в списке нет» ≠ «я этого не делал»:",
        "не отрицай факт, которого не проверял. Спросили про прошлое, чего тут нет → скажи честно, что",
        "не видишь в доступной истории, и предложи проверить источник (переписку, файл, историю чата).",
      ]
    : [];
  if (tasks.length === 0) {
    // Даже пустой список обязан нести границу: «пусто» без неё читается как «я ничего не делал».
    return boundary.length ? ["# Недавно выполненные задачи (§20)", ...boundary, "- (за это окно задач нет)"].join("\n") : "";
  }
