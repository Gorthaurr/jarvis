...
export class JarvisBrowser {
  ...
  private async _openChat(cdp: CdpConn, to: string, hint?: TgHint): Promise<{ chatTitle: string; peerId?: string; rect?: { x: number; y: number; w: number; h: number } }> {
    ...
    if (pick.action === "ask") {
      const list = pick.ranked.map((c) => c.title).join(" | ");
      throw new Error(`[tg-resolve] «${to}» — неоднозначно, наугад не шлю. Кандидаты: ${list}. Выбери того, кто по смыслу = «${to}» (учитывай транслитерацию: Герман≈Herman, падежи, никнеймы) и повтори с ТОЧНЫМ именем чата из списка.`);
    }
