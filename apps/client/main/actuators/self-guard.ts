 ...
 *   - ключевые контейнеры Windows/КриптоПро (%APPDATA%|%ProgramData%\Microsoft\Crypto, …\Crypto Pro) —
 *     приватные ключи сертификатов, в т.ч. квалифицированной подписи владельца;
 ...
 */
// 🔴 `self-patch.json` (волна I, ревью): состояние цикла САМОПРАВКИ — какая ветка, пройдена ли
// проверка, можно ли применять. Запись в него = подделка отметки «проверено»: подсунув stage
// «committed», можно было бы применить к рабочему коду то, что компилятор и тесты не видели, а
// владельцу показать сводку с чужим результатом проверки.
const POLICY_DATA_BASENAMES = new Set([
  // Выученные рецепты каналов: влияют на ДОВЕРЕННУЮ часть промпта («что вообще возможно»).
  // Без этого fs_write обходил бы все три рубежа записи (проба, скан инъекций, SSRF-гард).
  "app-recipes.json","profile.json", "tasks.json", "self-patch.json"]);
...
function isSecretPathNorm(normalized: string): boolean {
  ...
  // 🔴 КЛЮЧЕВЫЕ КОНТЕЙНЕРЫ Windows/КриптоПро — приватные ключи сертификатов, включая ключ
  // КВАЛИФИЦИРОВАННОЙ подписи физлица (на машине владельца такой сертификат есть: УЦ «Сертум-Про»,
  // СНИЛС/ИНН в субъекте, HasPrivateKey=True). Хранилища CAPI/CNG проверены живьём и реально
  // непусты: %APPDATA%\Microsoft\Crypto\RSA\<SID>\* (CAPI) и …\Microsoft\Crypto\Keys\* (CNG), плюс
  // машинные %ProgramData%\Microsoft\Crypto\{RSA\MachineKeys,Keys,DSS,SystemKeys,PCPKSP}. Имена
  // ключевых блобов — «<хэш>_<GUID>» БЕЗ расширения, поэтому ни одно правило выше их не ловило:
  // fs_read спокойно выгрузил бы ключевой материал в контекст модели, а fs_delete — сломал бы
  // подпись владельцу невосстановимо. Одна ветка кроет и пользовательское, и машинное хранилище.
  if (/[\\/]microsoft[\\/]crypto(?:[\\/]|$)/.test(normalized)) return true;
  // КриптоПро: собственные каталоги данных (%ProgramData%\Crypto Pro, %APPDATA%|%LOCALAPPDATA%\Crypto
  // Pro) — туда HDIMAGE-ридер («Директория») кладёт контейнеры-папки «имя.000». Сознательно НЕ трогаем
  // «C:\Program Files (x86)\Crypto Pro» — там дистрибутив CSP, не ключи, и блок мешал бы честно
  // ответить «КриптоПро установлен». Сами файлы контейнера (primary.key, masks.key, header.key,
  // name.key) отдельным правилом не перечислены: они уже попадают под общий денилист «*.key» выше.
  // ⚠️ ПРЕДЕЛ, о котором нельзя молчать: ридер «Реестр» держит контейнеры в
  // HKLM\SOFTWARE\[WOW6432Node\]Crypto Pro\Settings\Users\<SID>\Keys (ключ проверен, существует) —
  // это НЕ файловый путь, и путевым денилистом он не закрывается вовсе.
  if (/[\\/](?:appdata|programdata)[\\/](?:[^\\/]+[\\/])?crypto pro(?:[\\/]|$)/.test(normalized)) return true;
  return false;
}
...
/** Бросить, если пытаются ЧИТАТЬ секрет (.env, приватный ключ, контейнер подписи, креды) — не утекаем в контекст модели (§0). */
export function assertReadable(abs: string): void {
  if (isSecretPath(abs)) {
    // Причина названа КЛАССОМ, а не «.env»: под денилист попадают и приватные ключи, и креды браузера,
    // и ключевые контейнеры подписи — «это .env» было бы неправдой про них (§1: декларация = поведение).
    throw new Error(
      `защита секретов (§0): «${abs}» — секретный файл (ключи/креды/контейнер подписи), читать его в контекст модели нельзя.`,
    );
  }
}
