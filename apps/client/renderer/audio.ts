 ...
 *  Интеграционное ревью (#3): порог ОБЯЗАН быть больше серверного INACTIVITY_MS v3-стрима (8с) + сеть —
 *  иначе медленный (не мёртвый) стрим финализируется преждевременно ПОСРЕДИ фразы: сервер ещё пришлёт
 *  реальные чанки той же фразы (он абортит лишь на 8с), а клиент уже закрыл плеер → фраза рвётся на две
 *  + теряется carry-байт (сдвиг Int16 = шум). Зеркалит R5-логику PCM_ORPHAN_MS>INACTIVITY_MS. */
const DRAIN_IDLE_MS = 11_000;
