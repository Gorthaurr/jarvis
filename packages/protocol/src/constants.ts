/**
 * Константы протокола и семантики соединения (§5).
 * Несовпадение мажора PROTOCOL_VERSION → сервер отвечает ошибкой,
 * клиент показывает «требуется обновление» (рассинхрон громкий, не тихий).
 */

/** Мажорная версия протокола. Бампить при несовместимых изменениях контракта. */
export const PROTOCOL_VERSION = 1 as const;

/** Heartbeat: ping/pong каждые 15 c; два пропуска подряд → реконнект (§5). */
export const HEARTBEAT_INTERVAL_MS = 15_000;
export const HEARTBEAT_MAX_MISSES = 2;

/** Таймаут по умолчанию для ActionCommand, если сервер не задал свой (§5). */
export const DEFAULT_ACTION_TIMEOUT_MS = 15_000;

/**
 * Серверный потолок ожидания ЛЮБОГО skill.execute (реплей навыка, авто-макрос, input_batch).
 * ИНВАРИАНТ «нет двух писателей в GUI» (ревью Волны 3 #2, ревью фиксов #12): клиентский runSkill
 * сам укладывается в бюджет (JARVIS_SKILL_REPLAY_BUDGET_MS, деф/кламп ≤80с) с хвостовым перебегом
 * ≤ ~26с (executeStep ≤25с: hard-таймаут лаунчера app.launch / 2×UIA по 12с; ИЛИ короткий шаг +
 * один expect-опрос ≤20с OCR; + sleep ретрая ≤1.2с) → сервер обязан ждать СТРОГО дольше (130с),
 * иначе синтетический timeout запускает LLM-петлю (клики моделью) ПАРАЛЛЕЛЬНО ещё идущему реплею.
 * Кирпичи инварианта: кламп бюджета (apps/client/main/actuators/index.ts), skip expect-опроса при
 * исчерпанном бюджете и кламп retries (skill-runner/index.ts), кап текста REPLAY_TYPE_MAX_CHARS.
 */
export const SKILL_EXECUTE_SERVER_TIMEOUT_MS = 130_000;

/**
 * Кап длины input.type в ДЕТЕРМИНИРОВАННОМ реплее/берсте И в ЛЮБОМ шаге runSkill (интеграционное ревью,
 * 2-й проход #2). Сайдкарный typeText даёт себе люфт 5с + 120мс/символ (до 180с!), а отменить начатую
 * печать нельзя (cancel-токен — заглушка). 150 символов ⇒ typeText ≈ 5000+150×120 = 23с — укладывается в
 * «конверт одного шага», который держит инвариант «нет двух писателей» (серверный потолок 130с, бюджет
 * реплея ≤80с; бюджет гейтит СТАРТ шага, поэтому одиночный шаг обязан быть КОРОТКИМ по ДЛИТЕЛЬНОСТИ, а не
 * только по остатку — иначе длинный type-шаг жил бы дольше 130с и печатал бы в уже другое окно параллельно
 * LLM-петле). ⚠️ Прежний REPLAY_STEP_TYPE_MAX_CHARS=600 был ошибкой: 600 симв = 77с неотменяемой печати →
 * шаг, стартовавший у границы бюджета, перебегал за 130с. Длинный текст — не через навык (fs_write/office_*).
 */
export const REPLAY_TYPE_MAX_CHARS = 150;

/**
 * §режим выделения: потолок ожидания владельца в screen_selection{op:"start", waitMs} — ОДИН для схемы
 * инструмента (maximum), серверного клампа и клиентского актуатора. Контроль-4: сервер не клампил
 * модельное число, и при замёрзшем main-процессе клиента tool-вызов ждал бы waitMs+15 с без границы.
 */
export const SELECTION_MAX_WAIT_MS = 120_000;

/**
 * Таймаут отправки действия ПО ВИДУ команды. Корень бага «команда не выполнилась, хотя приложение
 * открылось»: серверный синтетический таймаут (15с) короче РЕАЛЬНОГО окна запуска на клиенте — холодный
 * `smartLaunch` (app-resolve.ts) делает Start-Sleep 1.5с + скан App Paths/Steam-манифестов/рекурсию по
 * Пуску, а для uri/steam поллит до 8с; hard-таймаут лаунчера 25с. Сервер успевал отрапортовать timeout,
 * пока клиент ещё (часто успешно) запускал → ЛОЖНЫЙ провал. Таймаут синтетический (session.ts) и unref'нут
 * — поднятие верхней границы happy-path (1-3с) не замедляет и голос не вешает (запуск идёт в фоне).
 */
export function actionTimeoutMs(kind: string): number {
  switch (kind) {
    case "skill.execute":
      return SKILL_EXECUTE_SERVER_TIMEOUT_MS; // ревью фиксов Волны 3 (#12): строго выше клиентского бюджета реплея
    case "app.launch":
      return 30_000; // холодный резолв + Start-Sleep + поллинг steam/uri; перекрывает hard-25с лаунчера +запас
    case "app.close":
    case "app.focus":
    case "browser.open":
      return 20_000; // close: Start-Sleep 1.2с + скан процессов; focus/open: резолв окна/shell-fallback
    case "wait.for":
      return 130_000; // §Волна2 (2.3): клиентское ожидание — до 120с поллинга + запас на транспорт
    case "screen.ocr":
      return 25_000; // §Волна2 (2.3): захват + OCR в сайдкаре (его внутренний таймаут 20с)
    case "screen.selection":
      return 25_000; // §выделение: захват региона + перцептивный хеш; op:"start" с waitMs считает свой потолок сам
    case "ui.snapshot":
      return 20_000; // §Волна2 (2.4): обход UIA-дерева сложного окна дольше дефолтных 15с
    case "gui.act":
      // W4 «Руки»: внутри — лестница поиска (фокус окна ≤8с + снапшот UIA ≤12с + OCR ≤20с) + действие + наблюдение
      // + ожидание признака. Клиент держит СВОЙ бюджет ACT_BUDGET_MS (45с) и честно останавливается на ступени,
      // на которую времени не осталось; серверный потолок ВЫШЕ клиентского с запасом на транспорт — иначе
      // УСПЕШНОЕ действие рапортовалось бы таймаутом, и ретрай модели ПОВТОРИЛ бы клик (класс, документированный
      // у input.click). Поднимаешь одно — пересчитай другое (apps/client/main/actuators/act.ts).
      return 60_000;
    case "fs.search":
      return 60_000; // ревью 2026-09-01: inContent на реальном дереве — минуты; клиентский бюджет 40с (fs-search-report) СТРОГО ниже
    case "fs.view":
      return 30_000; // §3.9 зрение на файл: рендер страницы PDF через python+PyMuPDF (холодный старт интерпретатора + внутренний потолок 25с)
    case "input.click":
    case "input.type":
    case "input.mouse":
    case "input.key":
    case "ui.invoke":
      // §Волна2 (2.1, ревью): бесшумная лестница клика (UIA до 12с) + fused-наблюдение (бюджет ~6.4с)
      // не влезали в дефолтные 15с → УСПЕШНОЕ действие рапортовалось таймаутом, ретрай ДУБЛИРОВАЛ клик.
      // input.key добавлен ревью 2026-09-01: с наблюдением-дельтой он делает ДВА обхода UIA (снимок
      // ДО + наблюдение ПОСЛЕ), и на дефолтных 15с запас до синтетического таймаута падал до ~1.2с —
      // а ложный таймаут на Enter означает ПОВТОРНУЮ отправку сообщения.
      return 30_000;
    default:
      return DEFAULT_ACTION_TIMEOUT_MS;
  }
}

/** Follow-up окно после конца TTS: микрофон горячий без повторного wake word (§10). */
export const FOLLOWUP_WINDOW_MS = 6_000;

/** Цель латентности до первого звука (§10). Метрика, не таймаут. */
export const TARGET_FIRST_AUDIO_MS = 800;
