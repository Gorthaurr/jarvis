...
> ⚠️ **ПОПРАВКИ 2026-07-28 (аудит «реальный Джарвис», ревью нашло протухшие строки — таблицы ниже
> ИСТОРИЧНЫ, сверяйтесь с `CLAUDE.md` и `docs/REAL_JARVIS_REVIEW_2026-07-28.md`):**
> • `brain/persona/acks.ts` (ButlerAcks) — **УДАЛЁН** («тихий финал» 2026-06-23), строка «✅» ниже неверна.
> • Боевой TTS = **Yandex (filipp)**, ElevenLabs — резерв; строка «ElevenLabs ✅ актив» неверна.
> • `integrations/calendar.ts` и `proactive/maps.ts` **НЕ существуют** (даже скелетами) — календарь/ETA не начаты.
> • Актуальный полный разрыв «что есть/чего нет» — в `docs/REAL_JARVIS_REVIEW_2026-07-28.md` (60 верифицированных гэпов).
